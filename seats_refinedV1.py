...
def main():
    ...
    number_of_rows, seat_layout, free_seats = read_database(db_file)

    booking_number, booking_name, booking_size = read_csv(csv_file)

    for i in range(booking_number):

        if free_seats == 0:                                                             # Zero available seats condition
                                                            # increase database count passengers refused by booking_size
        ...
        elif int(booking_size[i]) > free_seats:                # Insufficient remaining seats for this booking condition
                                                            # increase database count passengers refused by booking_size
        ...
        else:                                                                               # Booking accepted condition
                                                    # call seating allocation function and reduce no. of available seats
